...
**profiled with nsight compute** (`ncu --set full`, `q_proj`): **89% of dram peak, 91.4% achieved occupancy**. memory bound and nearly full, so occupancy is not the limiter at m=4096 and the `kv_proj` claim above stays a hypothesis until it is profiled too. the one real defect is an **8 way shared memory bank conflict** across 50% of shared load wavefronts: lane `j` reads `x` at byte `j*32`, so bank `(j*8) % 32` collapses 32 lanes onto 4 banks. padding the tile to a 36 byte stride gives `(j*9) % 32`, all distinct, and costs no occupancy. worth about 10%.
